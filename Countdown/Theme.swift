//
//  Theme.swift
//  Countdown
//
//  Created by Andre Hochschulte on 08.08.21.
//

import Foundation
import SwiftUI


class Theme: ObservableObject {
    
    @Published var primaryColor: Color = Color(.sRGB, red: 99/255, green: 101/255, blue: 241/255, opacity: 1.0)
    
    @Published var primaryTextColor: Color = Color.white
    
//    static var indigo900: Color = Color(.sRGB, red: 49/255, green: 46/255, blue: 129/255, opacity: 1.0)
//
//    static var indigo500: Color =
//
//    static var primaryColor: Color  = Color(.sRGB, red: 65/255, green: 65/255, blue: 81/255, opacity: 1.0)
//
//    static var primaryColorDark: Color = Color(.sRGB, red: 31/255, green: 41/255, blue: 55/255, opacity: 1.0)
//
//    static var primaryColorDarker: Color = Color(.sRGB, red: 17/255, green: 24/255, blue: 39/255, opacity: 1.0)
//
//    static var textColor: Color = Color.white
//
//    static var textColorSecondary = Color(.sRGB, red: 17/255, green: 24/255, blue: 39/255, opacity: 1.0)
}
